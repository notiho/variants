...
for profile, indices in indices_by_profile.items():
	...
	kr_numbers_counts = Counter(kr_numbers)
	
	if len(set(kr_numbers)) == 1:
		continue
	
	for kr_number in set(kr_numbers):
		print(f"Evaluating {kr_number}")
		
		train_indices = [i for i in indices if kr_numbers_by_sample_index[i] == kr_number]
		test_indices = [i for i in indices if kr_numbers_by_sample_index[i] != kr_number]
		...
		for i, layer_index, label in zip(test_indices, test_layer_indices, test_labels):
			...
			for j in range(num_targets):
				rsts.append((kr_number, kr_numbers_by_sample_index[i],
...
with open("regression_predictions_cross_document.csv", "w") as outfile:
	outfile.write("train_kr_number,test_kr_number,direction,index,profile,target,prediction,probability\n")
	for r in rsts:
